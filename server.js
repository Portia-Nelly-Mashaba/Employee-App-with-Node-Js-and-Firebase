...
app.post("/users", upload.single("image"), async (req, res) => {
    try {
      ...
      const auth = getAuth();
      const resetLink = await auth.generatePasswordResetLink(req.body.email);
  
      // Use EmailJS to send the reset password link
      const emailData = {
      ...
      // Save the user in Firestore (optional)
      const user = {
        firstname: req.body.firstname,
        lastName: req.body.lastName,
        identity: req.body.identity,
      ...
      res.send({
        success: true,
        message: "User created and notification sent successfully!",
        id: docRef.id,
      });
    } catch (error) {
...
//       // Construct the user object from the request body
//       const user = {
//         firstname: req.body.firstname,
//         lastName: req.body.lastName,
//         identity: req.body.identity,
...
//         role: req.body.role,
//         image: photoBase64, // Store image in Base64 format
//         createdAt: admin.firestore.FieldValue.serverTimestamp(), // Firebase timestamp
//       };
  
//       // Add user to the "users" Firestore collection
//       const docRef = await db.collection("users").add(user);
  
//       res.send({ success: true, id: docRef.id }); // Respond with success and user ID
//     } catch (error) {
